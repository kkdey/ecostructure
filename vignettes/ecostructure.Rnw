...
disp <- CreateGlobalDispersionFields(taxonomic_counts,
              shapefiles_dir = system.file("extdata",
                    "ne_50m_admin_0_countries", package = "ecostructure"))
@


<<mapsdisp_fields, eval=FALSE>>=
dispersion.field <- disp
...
<<mask_climate_mask, eval=FALSE>>=
mask_climate_raster_by_GoM(motif_map_raster, climate_raster, quant = 0.10)
@
